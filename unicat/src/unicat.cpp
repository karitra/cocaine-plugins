...
namespace detail {
    ...
    auto
    abort_deferred(Deferred&& deferred, Log&& log, const std::string& log_message, const std::string& deferred_message) -> void {
    ...
    template<typename R>
    auto
    unpack(const unicorn::versioned_value_t value) -> std::tuple<cu::version_t, R> {
        ...
        if (value.exists() && value.value().convertible_to<R>()) {
            return std::make_tuple(value.version(), value.value().to<R>());
        }
...
struct on_read_t : public unicat::async::read_handler_t {
    ...
    auto on_read(std::future<unicorn::versioned_value_t> fut) -> void override {
        try {
            dbg("on_read: before future.get()");
            auto data = fut.get();

            auto meta = auth::metainfo_t{};
            auto version = cu::version_t{};

            std::tie(version, meta) = detail::unpack<auth::metainfo_t>(data);
...
private:
    auto on_read(auth::metainfo_t metainfo, const cu::version_t version = cocaine::unicorn::not_existing_version) -> void {
        ...
        auto on_verify = [=] (std::error_code ec) -> void {
            if (ec) {
                return detail::abort_deferred(*promise, backend->logger(),
                    "failed to complete 'write' operation", "Permission denied");
            }

            dbg("before write metainfo");
            backend->async_write_metainfo(entity, version, metainfo, std::make_shared<on_write_t>(promise));
        };

        dbg("verify write: " << metainfo);
        backend->async_verify_write(entity, cu::async::verify_handler_t{identity, on_verify});
    }
};
            ...
            for(const auto& srv : services) {
                ...
                for(const auto entity : entities) {
                    ...
                    auto on_verify = [=] (std::error_code ec) -> void {
                        ...
                        dbg("before read metainfo");
                        backend->async_read_metainfo(entity,
                            std::make_shared<on_read_t<Event>>(backend, entity, identity, alter_data, promise));
                    };

                    dbg("async verify read for " << entity);
                    backend->async_verify_read(entity, cu::async::verify_handler_t{identity, on_verify});
                } // for entities
            } // for services
        } // for schemes

        detail::wait_all(completions);
